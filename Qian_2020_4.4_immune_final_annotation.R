...
draw_dotplot(blueprint_CRC_immune_cleaned, Markers_Zhang_2023_FigS1, 
             group = "annotation", suffix = "_immune_Final_Annotation_markers_Zhang_2023",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 12, fig_height = 11)

draw_dotplot(blueprint_CRC_immune_cleaned, rbind(Markers_Qi_2022_Tcell_top5,
                                                 Markers_Qi_2022_Bcell_top5, 
                                                 Markers_Qi_2022_macrophage_top5), 
             group = "annotation", suffix = "_immune_Final_Annotation_markers_Qi_2022",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 12, fig_height = 22)


draw_dotplot(blueprint_CRC_immune_cleaned, subset(Markers_Ji_2024, !cluster %in% c("Epithelial", "Ecs", "Fibroblasts")), 
             group = "annotation", suffix = "_immune_Final_Annotation_markers_Ji_2024",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 12, fig_height = 15)

# Marker genes
blueprint_CRC_immune_cleaned[["joined"]] <- JoinLayers(blueprint_CRC_immune_cleaned[["RNA"]])
DefaultAssay(blueprint_CRC_immune_cleaned) <- "joined"
blueprint_CRC_immune_cleaned <- SetIdent(blueprint_CRC_immune_cleaned, value = "annotation")
blueprint_CRC_immune_cleaned_markers <- FindAllMarkers(
  blueprint_CRC_immune_cleaned, logfc.threshold = 0.25, only.pos = TRUE, min.pct = 0.25)
saveRDS(blueprint_CRC_immune_cleaned_markers, file = "R_objects/Qian_2020/blueprint_CRC_immune_cleaned_markers.rds")

blueprint_CRC_immune_cleaned_markers_top5 <- blueprint_CRC_immune_cleaned_markers %>%
  group_by(gene) %>% arrange(desc(avg_log2FC)) %>% slice(1) %>%
  group_by(cluster) %>% arrange(p_val, desc(avg_log2FC)) %>% slice(1:5) %>%
  mutate(gene = factor(gene, unique(.$gene))) 
  
draw_dotplot(blueprint_CRC_immune_cleaned, blueprint_CRC_immune_cleaned_markers_top5,
             group = "annotation", suffix = "_immune_Final_Annotation_top5_markers",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 12, fig_height = 23)

draw_dotplot(blueprint_CRC_immune_cleaned, 
             subset(blueprint_CRC_immune_cleaned_markers_top5, 
                    cluster %in% subset(annotation_group, group %in% c("T/NK", "B/PlasmaB"))$annotation),
             group = "annotation", suffix = "_immune_Final_Annotation_top5_markers_T_B",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 13, fig_height = 16)

draw_dotplot(blueprint_CRC_immune_cleaned, 
             subset(blueprint_CRC_immune_cleaned_markers_top5, 
                    cluster %in% subset(annotation_group, group == "Myeloid")$annotation),
             group = "annotation", suffix = "_immune_Final_Annotation_top5_markers_Myeloid",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 13, fig_height = 13)

Markers_selected <- list(
  immune = c("PTPRC"),
  Tcells = c("CD3D", "CD3E", "CD3G"),                                # T cells
  T_CD4 = c("CD4"),   # CD4+ T cells
  T_CD4_naive = c("IL7R", "CCR7", "LEF1", "SELL", "TCF7"),
  T_CD8 = c("CD8A", "CD8B"),        # CD8+ T cells
  gdT = c("TRGC1", "TRGC2", "TRDC"),
  NK = c("KLRF1", "KLRC1"),            # NK cells
  Bcells = c("MS4A1", "CD79A", "CD79B", "BANK1"),
  B_naive	= c("TCL1A", "IGHD", "IGHM"),
  B_memory = c("CD27", "AIM2"),
  B_GC_independent = c("CD38"),
  PlasmaB = c("JCHAIN", "MZB1"),
  PlasmaB_IgA = c("IGHA1", "IGHA2"),
  PlasmaB_IgG = c("IGHG1", "IGHG2"),
  PlasmaB_mature = c("PRDM1"),
  Macrophage = c("C1QA", "C1QB", "MRC1"),     # Macrophage
  "SPP1+ TAM" = c("MARCO", "SPP1"),
  "C1QC+ Macrophage" = c("C1QC"),
  "LYVE+ Macrophage" = c("LYVE1"),
  Monocyte = c("S100A8", "S100A9", "S100A12", "CD14", "FCGR3A"),
  Neutrophil = c("FCGR3B", "CXCL1", "CXCL8", "CSF3R"),
  cDC1 = c("CLEC9A", "XCR1"),
...
draw_dotplot(blueprint_CRC_immune_cleaned, Markers_selected,
             group = "annotation", suffix = "_immune_Final_Annotation_markers_selected",
             folder = "Qian_2020/Final_annotation",
             pal = rev(brewer.pal(11, "RdBu")), fig_width = 13, fig_height = 16)
...
ncell_by_sample_annot <- blueprint_CRC_immune_cleaned@meta.data %>%
  ...
  left_join(annotation_group, by = "annotation") %>%
  mutate(annotation = factor(annotation, levels(blueprint_CRC_immune_cleaned$annotation)))
...
prop_by_sample_annot <- celltype_prop_ttest$props$Proportions %>%
  ...
  mutate(clusters = factor(clusters, levels(blueprint_CRC_immune_cleaned$annotation)))
